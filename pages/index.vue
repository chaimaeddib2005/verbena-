...
export default {
  ...
  methods: {
    ...
    processSpecificShortcodes(content) {
      ...
      processed = processed.replace(shortcodePatterns.caption, (match, captionContent) => {
        const captionText = captionContent.replace(/<img[^>]*>/gi, '').trim();
        return captionText ? `(${captionText})` : '';
      });
      
      // Process [gallery] shortcodes
      processed = processed.replace(shortcodePatterns.gallery, () => '[Galerie d\'images]');
      
      // Process [video] shortcodes
      processed = processed.replace(shortcodePatterns.video, () => '[Vidéo]');
      
      // Process [audio] shortcodes
      processed = processed.replace(shortcodePatterns.audio, () => '[Audio]');
      
      // Process [embed] shortcodes
      processed = processed.replace(shortcodePatterns.embed, (match, embedContent) => {
      ...
      processed = processed.replace(shortcodePatterns.button, (match, buttonText) => {
        return buttonText.trim() || '[Bouton]';
      });
      
      // Process column shortcodes
      processed = processed.replace(shortcodePatterns.columns, (match, columnContent) => {
    ...
    removeShortcodes(content) {
      ...
      return content.replace(genericPattern, '');
    },
    ...
    async fetchTopCategories() {
      try {
        ...
        const categoriesWithPosts = await Promise.all(
          categoriesResponse.slice(0, 10).map(async (category) => {
            try {
